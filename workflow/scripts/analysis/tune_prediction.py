...
from sklearn.covariance import OAS, EmpiricalCovariance, ShrunkCovariance, MinCovDet, LedoitWolf
...
def create_model(trial):
    ...
    if TARGET != 'PTD':
        ...
        model_options = {'bnb': BNB, 'dtc': DTC,'lda': LDA,'lrc': LRC,'qda': QDA,'rfc': RFC, 'svc': SVC, 'knn': KNN, 'nn': MLP }
    covariance_options  = {'OAS': OAS, 'EC':EmpiricalCovariance, 'SC': ShrunkCovariance, 'LW':LedoitWolf, 'none': None}   
    ...
    if model_name == 'bnb':
        ...
        kwargs["max_features"]      = trial.suggest_categorical("max_features", [None, 'sqrt', 'log2'])
        ...
        if SUBSET == 'all':
            ...
            kwargs["solver"]            = trial.suggest_categorical("solver_lda", ['lsqr', 'eigen', 'svd'])
        if kwargs["solver"] == 'svd':
            selected_cov  = 'none'
        elif kwargs["solver"] == 'eigen':
            selected_cov                    = trial.suggest_categorical("covariance_estimator_e", ['OAS', 'SC', 'LW'])
        elif kwargs["solver"] == 'lsqr':
            selected_cov    = trial.suggest_categorical("covariance_estimator_lsqr", covariance_options.keys())
        if selected_cov != 'none':
            kwargs["covariance_estimator"]  = covariance_options.get(selected_cov)()
        else:
            kwargs["covariance_estimator"] = None
    elif model_name == 'lrc':
        kwargs["penalty"] = trial.suggest_categorical("penalty", ["l1", "l2", "elasticnet", None])
        kwargs["warm_start"]            = trial.suggest_categorical("warm_start", [False, True])
        if kwargs["penalty"] != None:
            kwargs["multi_class"] = trial.suggest_categorical("multi_class_l1", ["auto", "ovr"])
            kwargs["C"] = trial.suggest_float("C", 1e-3, 3, log=True)
            if kwargs["penalty"] != "elasticnet":
                kwargs["l1_ratio"] = None
                if kwargs["penalty"] == "l1":
                    kwargs["solver"] = trial.suggest_categorical("solver_l1", ["liblinear", "saga"])
                elif kwargs["penalty"] == "l2":
                    kwargs["solver"] = trial.suggest_categorical("solver_l2", ["lbfgs", "newton-cg", "newton-cholesky", "sag", "saga", "liblinear"])
                else:
            ...
            kwargs["solver"] = trial.suggest_categorical("solver_none", ["lbfgs", "newton-cg", "saga"]) # "sag"
            kwargs["multi_class"] = trial.suggest_categorical("multi_class_none", ["auto", "ovr", "multinomial"])       
        ...
        kwargs["criterion"]             = trial.suggest_categorical("criterion", ['gini', 'entropy', 'log_loss'])
        kwargs["min_samples_split"]     = trial.suggest_float("min_samples_split", 0.01, 0.99, log=True)
        kwargs["min_samples_leaf"]      = trial.suggest_float("min_samples_leaf", 0.01, 0.99, log=True)
        kwargs["max_features"]          = trial.suggest_categorical("max_features", ['sqrt', 'log2', None])
        kwargs["min_impurity_decrease"] = trial.suggest_float("min_impurity_decrease", 0.001, 1, log=True)
        ...
        if kwargs["bootstrap"]:
            ...
            kwargs["class_weight"]      = trial.suggest_categorical("class_weight_b", ['balanced_subsample', None])
        else:
            kwargs["class_weight"]      = trial.suggest_categorical("class_weight", ['balanced', None])
        ...
        kwargs["class_weight"]  = trial.suggest_categorical( "class_weight_svc", [None, 'balanced'])
    elif model_name == 'knn':
        kwargs["algorithm"] = trial.suggest_categorical("algorithm", ["auto", "ball_tree", "kd_tree", "brute"])
        if kwargs["algorithm"] == "ball_tree":
            kwargs["weights"] = trial.suggest_categorical("weights_kd", ["uniform","distance", None])
            kwargs["metric"] = trial.suggest_categorical("metric_ball",
                                                         [
                                                             "euclidean",              
                                                             "l2",
                                                             "minkowski",
            ...
            kwargs["metric"] = trial.suggest_categorical( "metric_kd",
                                                         [
                                                             "euclidean",
                                                             "l2",
                                                             "minkowski",
            ...
            kwargs["weights"] = trial.suggest_categorical( "weights_brute", 
                                                          [
                                                              "distance", None
                                                          ]
                                                         )
 
            kwargs["metric"] = trial.suggest_categorical( "metric_brute",
                                                         [
                                                             "cosine",
                                                             "euclidean",
                                                             "manhattan",
            ...
            kwargs["metric"] = trial.suggest_categorical( "metric_auto", 
                                                         [
                                                             "euclidean", 
                                                             "manhattan", 
                                                             "minkowski"
        ...
        kwargs["activation"]                = trial.suggest_categorical("activation", ['logistic', 'tanh', 'relu'])
        kwargs["solver"]                    = trial.suggest_categorical("solver", ['sgd', 'adam']) #,'lbfgs'])
        ...
        if kwargs["solver"] == "adam":
            ...
            if not kwargs.get("warm_start", False):
                ...
                if kwargs["early_stopping"]:
                    kwargs["validation_fraction"]  = trial.suggest_float("validation_fraction", 0.1, 0.4)
            kwargs["learning_rate_init"]    = trial.suggest_float("lr_init_adam", 0.00001, 1, log=True)
            ...
            kwargs["learning_rate_init"]    = trial.suggest_float("lr_init_sgd", 0.000001, 0.1, log=True)
            kwargs["learning_rate"]         = trial.suggest_categorical("learning_rate", ['constant', 'invscaling', 'adaptive'])
            ...
            if kwargs["momentum"] > 0 :
                kwargs["nesterovs_momentum"] = trial.suggest_categorical("nesterovs_momentum", [True, False])
...
def objective(trial):
    ...
    class_weights = compute_class_weight(class_weight='balanced', classes=classes, y=y_outer)
    ...
    for step in range(n_train_iter):
        x_train, x_val, y_train, y_val = train_test_split(x_outer, y_outer, test_size=0.2, stratify=y_outer)
        if TARGET == "PTD":
...
def get_parameters():
    ...
    print(
        "Optimizing took %.2f seconds for %d trials." % ((time() - start), TRIALS))
...
#         scorer = make_scorer(roc_auc_score, needs_proba=True)
    test_score, test_perm, test_pval = ps_permutation_test_score(
            model, [x_train, x_test], [y_train, y_test], scorer=scorer, n_permutations=n_perm, random_state=None)
...
#    score_dict = {"number": params["number"]}
    ...
    if TARGET == "PTD":
        score_dict.update({
            "train_score": roc_auc_score(y_train, model.predict_proba(x_train)[:,1], average="weighted"),
...
#     score_dict["test_score"]     = test_score
    ...
    pred_df = pd.DataFrame({pred_name: y_prob,
                            'Target': y_test})
...
if __name__=='__main__':
   ...
   # """
    ...
    pred_dir = path + f'out/analysis/predictions/'

    score_df = pd.DataFrame.from_records(scores)
    score_df.to_csv(score_file, sep='\t', float_format='%.5f', index=False, mode='a')#, header=not file_exists)
